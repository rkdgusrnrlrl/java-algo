...
public class TestMtree {
    ...
    * 3. full binary me.dakbutfly.tree 의 경우 N개의 노드를 가진경우 높이는 logN + 1 임
    * */

    /*
    * me.dakbutfly.tree.MtreeNode 명세
    * 필드
    *   - me.dakbutfly.tree.MtreeNode left 왼쪽 노드
    *   - me.dakbutfly.tree.MtreeNode right 오른쪽 노드
    *   - T value 노드의 값
    *  메서드
    *   - me.dakbutfly.tree.MtreeNode 생성자
    *     - T value : 해당 노드의 값
    *   - putLeft(me.dakbutfly.tree.MtreeNode<T> left) : void => 해당 노드에 left 노드를 할당함
    *   - getLeft() : me.dakbutfly.tree.MtreeNode<T> => 해당 노드에 left 노드 리턴
    *   - putRight(me.dakbutfly.tree.MtreeNode<T> right) : void => 해당 노드에 right 노드를 할당함
    *   - getRight() : me.dakbutfly.tree.MtreeNode<T> => 해당 노드에 right 노드 리턴
    * */
    ...
    * */
    ...
    public void Mtree에_left와_right_필드_셋하고_확인하기() throws Exception {
        ...
        //then
        assertThat(integerMtreeNode.getLeft(), CoreMatchers.is(four));
        assertThat(integerMtreeNode.getRight(), CoreMatchers.is(six));
    ...
    * */
    ...
    public void Mtree_노트_탐색_테스트() throws Exception {
        ...
        * */
        ...
        //then
        assertThat(rootFive.getLeft().getLeft().getRight(), CoreMatchers.is(tree));
        assertThat(rootFive.getRight().getRight().getLeft(), CoreMatchers.is(eight));
    }

    // findNodeInteger 해당 값이 노드 값은integerMtreeNode int 값으로 비교해서 me.dakbutfly.tree 선택
    ...
    public void 값이_5인_Root노드에서_findNode로_8을_인자로_넘겨주면_rightNode를_넘겨줌() throws Exception {
        ...
        assertThat(shouldBeEight, CoreMatchers.is(eight));
    }
    ...
    public void Root_에서_다른_노드로_가는_경로는_하나뿐임() throws Exception {
        ...
        for (int i = 0; i < expect.size(); i++) {
            assertThat(path.get(i), CoreMatchers.is(expect.get(i)));
        }
    }
